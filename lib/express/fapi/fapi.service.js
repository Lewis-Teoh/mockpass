...
class FapiService {
  constructor(idp) {
    ...
    // Maps access_token → { profile, authRequest } for the userinfo endpoint
    this.accessTokenStore = new ExpiryMap(30 * 60 * 1000)
  }

  async handleParRequest(req) {
    verifyParRequestBody(req, this.idp)
    const parEndpoint =
  ...
  handleCustomProfileAuthorizationRequest(req) {
    ...
    // CorpPass-only fields (uen, sub_uen, cp_role) are pass-through; ignored for Singpass
    const profile = { nric, uuid }
    if (this.idp === 'corpPass') {
      profile.uen = uen
      if (subUen) profile.subUen = subUen
      if (cpRole) profile.cpRole = cpRole
    }
  ...
  async generateIdToken(req, authRequest) {
    ...
    const id_token =
      this.idp === 'corpPass'
        ? {
            ...baseClaims,
            // EntityInfo carried in id_token for CorpPass; full auth_info lives in userinfo
            EntityInfo: {
              CPEntID: profile.uen || '',
              CPEntStatus: 'Registered',
              CPNonUEN_Country: '',
              CPNonUEN_RegNo: '',
              CPNonUEN_Name: '',
            },
    ...
    const access_token = crypto.randomBytes(64).toString('base64url')
    if (this.idp === 'corpPass') {
      // Store mapping so the userinfo endpoint can resolve back to the profile
      this.accessTokenStore.set(access_token, { profile, authRequest })
    }
    return {
      access_token,
  ...
  // Only registered as a route when this.idp === 'corpPass' (see fapi.controller.js)
  async handleUserInfoRequest(req) {
    ...
    const stored = this.accessTokenStore.get(access_token)
    if (!stored) throw new Error('Invalid or expired access token')
    const { profile, authRequest } = stored

    const issuer = FapiUtils.getFapiOpenIdConfiguration(req).issuer
    const expires_in = FapiUtils.idTokenConfiguration.TOKEN_EXPIRY

    // Configurable via env vars; profile-level overrides take precedence
    const cpESrvcId =
      process.env.MOCKPASS_CORPPASS_ESRVCID || 'GPCONNECT_DIGITAL_SERVICE_ID'
    const cpRole =
      profile.cpRole || process.env.MOCKPASS_CORPPASS_ROLE || 'GPCONNECT_USER'
    // For entities with no Sub-UEN configured, CPEntID_SUB === parent UEN
    const cpEntIdSub = profile.subUen || profile.uen || ''
    ...
    // Sign with ASP private key (encryption can be layered on later — for dev,
    // signed JWS is sufficient to validate the GPConnect parser)
    const signingKid = FapiUtils.FAPI_ASP_SIGNING_KID
    const fapiPrivateJwks = JSON.parse(
      fs.readFileSync(
        path.resolve(__dirname, FapiUtils.FAPI_ASP_PRIVATE_JWKS_PATH),
    ...
    const signedUserInfo = await new jose.CompactSign(
      new TextEncoder().encode(JSON.stringify(userinfoPayload)),
    )
      .setProtectedHeader(signedProtectedHeader)
      .sign(signingKey)
...
function verifyParRequestBody(req, idp) {
  const {
    response_type,
    scope,
    state,
...
function verifyAllowedScopes(scope, idp) {
  assert(scope, 'Scope is required')
  const scopes = scope.split(' ')
  const allowedScopes =
    idp === 'corpPass'
      ? [...Object.values(FapiUtils.SupportedScope), 'authinfo', 'person_info']
      : Object.values(FapiUtils.SupportedScope)
