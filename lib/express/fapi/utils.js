...
function getFapiOpenIdConfiguration(req, idp) {
  const issuer = getIssuerFromRequest(req)
  const isCorpPass =
    (idp || inferIdpFromRequest(req)).toLowerCase() === 'corppass'
  ...
  const config = {
    ...
    scopes_supported: isCorpPass
      ? [...Object.values(SupportedScope), 'authinfo', 'person_info']
      : Object.values(SupportedScope),
    subject_types_supported: ['public'],
    claims_supported: isCorpPass
      ? [...Object.values(SupportedClaims), 'auth_info', 'EntityInfo']
      : Object.values(SupportedClaims),
  ...
  if (isCorpPass) {
    config.userinfo_endpoint = `${issuer}/userinfo`
    config.userinfo_signing_alg_values_supported = ['ES256']
  }
